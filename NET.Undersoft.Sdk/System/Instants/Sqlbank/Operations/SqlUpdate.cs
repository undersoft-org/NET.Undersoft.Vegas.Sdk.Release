...
namespace System.Instants.Sqlbank
{
    public class SqlUpdate
    {
        ...
        public Album<Album<IFigure>> Update(IFigures table, bool keysFromDeck = false, bool buildMapping = false, bool updateKeys = false, string[] updateExcept = null, BulkPrepareType tempType = BulkPrepareType.Trunc)
        {
            try
            {
                IFigures tab = table;
        ...
        public Album<Album<IFigure>> BatchUpdate(IFigures table, bool keysFromDeck = false, bool buildMapping = false, bool updateKeys = false, string[] updateExcept = null, int batchSize = 250)
        {
            try
            {
                IFigures tab = table;
                ...
                foreach (IFigure ir in table)
                {
                    ...
                    if (count >= batchSize)
                    {
                        ...
                        Album<Album<IFigure>> bIFigures = afad.ExecuteUpdate(sb.ToString(), tab);
                        if (nSet.Count == 0)
                            nSet = bIFigures;
                        else
                            foreach (Album<IFigure> its in bIFigures.AsValues())
                ...
                Album<Album<IFigure>> rIFigures = afad.ExecuteUpdate(sb.ToString(), tab, true);

                if (nSet.Count == 0)
                    nSet = rIFigures;
                else
                    foreach (Album<IFigure> its in rIFigures.AsValues())
                    {
                        if (nSet.Contains(its))
                        {
                            nSet[its].Put(its.AsValues());
