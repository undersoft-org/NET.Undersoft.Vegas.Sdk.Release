...
namespace System.Instants.Sqlbank
{
    public class SqlAdapter
    {
        ...
        public InstantFigures ExecuteInject(string sqlqry, string tableName,
                                      IDeck<string> keyNames = null)
        {
            try
            {
                ...
                InstantFigures it = dr.InjectRead(tableName, keyNames);
                sdr.Dispose();
                cmd.Dispose();
                return it;
            }
        ...
        public InstantFigures ExecuteInject(string sqlqry, string tableName = null)
        {
            SqlCommand cmd = new SqlCommand(sqlqry, _cn);
            cmd.Prepare();
            if (_cn.State == ConnectionState.Closed)
            ...
            IDataReader sdr = cmd.ExecuteReader();
            SqlReader<IFigures> dr = new SqlReader<IFigures>(sdr);
            InstantFigures it = dr.InjectRead(tableName);
            sdr.Dispose();
            cmd.Dispose();
            return it;
        }      

        public Album<Album<IFigure>> ExecuteUpdate(string sqlqry, IFigures cards, bool disposeCmd = false)
        {
            if (_cmd == null)
        ...
        public Album<Album<IFigure>> ExecuteInsert(string sqlqry, IFigures cards,  bool disposeCmd = false)
        {
            if (_cmd == null)
                _cmd = _cn.CreateCommand();
            SqlCommand cmd = _cmd;
