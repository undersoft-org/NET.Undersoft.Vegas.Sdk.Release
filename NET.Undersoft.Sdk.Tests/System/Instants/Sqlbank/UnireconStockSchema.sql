...
IF EXISTS(select * from sys.databases where name='UnireconStock')
	ALTER DATABASE UnireconStock SET SINGLE_USER  WITH ROLLBACK IMMEDIATE;
	GO
IF EXISTS(select * from sys.databases where name='UnireconStock')
	DROP DATABASE UnireconStock
	GO

CREATE DATABASE UnireconStock
GO
